...
export const baseAnalyticsCalc: BaseAnalyticsCalc =
  (analyticsCalcFx) => (startDate) => (flow) => (history) => {
    ...
    const flowEndAtValue = flow.endAt
      ? getMidnight(flow.endAt).valueOf()
      : null;
    ...
    const historyCountStackedByAccmulateId = historyWithAccumulateId.map(
      (it) => {
    ...
    return Array.from({
      ...
      .reduce<
        Omit<
          AnalyticsResultObject & { id: number },
      ...
      >((result, _, day) => {
        ...
        return result.concat({
          result: analyticsCalcFx(targetHistory, flow),
          accumulateId,
          id: day,
        });
      }, [])
